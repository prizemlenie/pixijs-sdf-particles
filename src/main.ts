import { Application, Assets, Sprite, WebGLRenderer } from 'pixi.js';
...
(async () => {
    ...
    const noiseTexture = await Assets.load('perlin_color.png');
    noiseTexture.source.addressMode = 'repeat';
    const colorLUT = await Assets.load('color_lut.png');
    ...
    const { textures: textMasks } = await generateTextMask({
        text: 'BEAT',
        resolution: {
